<template>
    <v-card outlined>
        <v-card-title>
            Device # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="DeviceType" v-model="item.deviceType" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/devices/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
